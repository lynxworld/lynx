...
case class LynxTime(offsetTime: OffsetTime) extends LynxTemporalValue with LynxComponentTime with LynxComponentTimeZone {
  ...
  var microsecond: Int = offsetTime.getNano * Math.pow(0.1, 6).toInt
  var millisecond: Int = (offsetTime.getNano * Math.pow(0.1, 3) % Math.pow(10, 3)).toInt
...
object LynxTime {
  ...
  def parse(timeStr: String): LynxTime = LynxTime(OffsetTime.parse(timeStr))
  ...
  def parse(map: Map[String, Any]): LynxTemporalValue = {
    ...
    val zoneId = getZone(map.getOrElse("timezone", "Z").asInstanceOf[String])
