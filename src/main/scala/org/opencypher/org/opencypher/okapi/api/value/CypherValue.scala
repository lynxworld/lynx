...
object CypherValue {
  ...
  //<--BLOB
  implicit class CypherBlob(val value: Blob) extends AnyVal with PrimitiveCypherValue[Blob] {
    override def cypherType: CypherType = CTBlob
  }
  ...
  //<--BLOB
  object CypherBlob extends UnapplyValue[Blob, CypherBlob]
