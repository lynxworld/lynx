...
class CypherRemoveTest extends TestBase {
  ...
  def removeNodeProperty(): Unit ={
    ...
    Assertions.assertEquals(Seq("Oliver Stone", "male", 18), res("n").asInstanceOf[TestNode].props.values.toSeq.map(_.value))
  }
  ...
  def removeNodeLabel(): Unit ={
    ...
    Assertions.assertEquals(Seq("person"), res("n").asInstanceOf[TestNode].labels.map(_.value))
  }
  ...
  def removeRelationshipProperties(): Unit ={
    ...
    Assertions.assertEquals(Seq(5, 20), res("r").asInstanceOf[TestRelationship].props.values.map(_.value).toSeq)
  }
}
