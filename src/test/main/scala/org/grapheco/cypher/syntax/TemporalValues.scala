...
class TemporalValues extends TestBase{
  ...
  def accessDurationEx3():Unit={
    ...
    expectResult.foreach(f=>{println(f._1,f._2)})
  }
  ...
  def examples():Unit={
    ...
    cypher.foreach(e=>{
      try{
        ...
        case ex:AssertionError=>{
          logger.error("Find AssertError for cypher:"+e("cypher"))
          logger.error(ex.toString+"\n\n")
          flag+=1
        }
        case ex:Exception=>{
          logger.error("Find ScalaError for cypher:"+e("cypher"))
          logger.error(ex.toString+"\n\n")
          flag+=1
        }
      }
    ...
    if(flag>0){
      throw new Exception("Find error cypher "+flag+"/"+cypher.length)
    }
  }
