...
import org.opencypher.lynx.LynxSession.EMPTY_GRAPH_NAME
...
import org.opencypher.okapi.api.graph.{CypherQueryPlans, PropertyGraph, _}
...
object LynxRecords {
  def empty(header: RecordHeader = RecordHeader.empty) = new LynxRecords(header, LynxDataFrame.empty)
}

class LynxResult(physicalPlan: PhysicalOperator, maybeLogicalPlan: Option[LogicalOperator]) extends CypherResult {
...
object LynxResult {
  def empty()(implicit session: LynxSession) = new CypherResult() {
    override type Records = LynxRecords
    override type Graph = LynxPropertyGraph
